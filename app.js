...
app.post('/signin', captcha,async (req, res, next) => {
...
app.post('/login' , captcha , async (req, res, next) => {
    ...
    signIn.findOne({ email}, async function (err, result) {
        ...
        if (await bycrypt.compare(password, result.password)) return res.send({ success: true, msg: "login succeed", token })
...
app.put('/updatedids', async (req, res, next) => {
...
app.post('/forgotPassword', captcha, (req, res, next) => {
    crypto.randomBytes(32, (err, buffer) => {
        if (err) return res.send({ success: false, msg: "error in generating your id." })
        const token = buffer.toString('hex')
...
app.post('/newPassword', captcha, async (req, res, next) => {
